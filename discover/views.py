    ...
    try:
        if req.method == "GET":
            ...
            serializer = DiscoverSerializer(
                discoveries, many=True, context={'user_id': req.user.id} #context to pass arguments to serializer class
                )
...
def getDetailDiscover(req, disc_id):
    try:
        ...
        isLiked =  Like.objects.filter(discover_item_id = discover.id,liker_id = profile.id).exists()
        print(discover.poster)
 
        serializer = DiscoverSerializer(discover, many=False,context={'user_id': req.user.id})
...
def makeEditorChoice(req, disc_id):
    ...
    try:
        if user_data["isadmin"] == True:
            ...
            discovereditor = DiscoverEditorChoice.objects.filter(discover_item_id__exact=disc_id)
...
def getMyDiscoveries(req):
    ...
    try:
        ...
        if user.is_authenticated:
           ...
           discoveries =  Discover.objects.filter(is_deleted=False,poster__user__id = user.id).order_by("-created_time")
           serializer = DiscoverSerializer(
                discoveries, many=True, context={'user_id': req.user.id} #context to pass arguments to serializer class
                )
           return Response (
